...
// var BKTests = []BKTest{
// 	{"1k1r4/pp1b1R2/3q2pp/4p3/2B5/4Q3/PPP2B2/2K5 b - - 0 1", engine.Move{enc: engine.NewMove(engine.D1, engine.D1, 0)}},
// 	{"3r1k2/4npp1/1ppr3p/p6P/P2PPPP1/1NR5/5K2/2R5 w - - 0 1", engine.Move{engine.d5, engine.NoSquare, engine.Pawn, engine.NoPiece, engine.NoPiece}},
...
func Run(timePerCase int, hashSize uint64) (totalCorrect, totalRun int) {
	...
	for _, testCase := range testCases {
		...
		out := engine.MoveToString(board.StartSearchNoDepth(time.Now(), searchCancelChannel))

		passed := false

		for _, correctMove := range testCase.bestMoves {
			if correctMove == out {
				passed = true
				break
			}
		...
		if passed {
			...
			fmt.Printf("%s FAILED: wanted %v, got %s\n", testCase.id, testCase.bestMoves, out)
		}
	}
...
func GetTests() (retval []BKTest) {
	...
	for scanner.Scan() {
		line := scanner.Text()
		// Split the line into the fen string, best moves, and test ID
		parts := strings.Split(line, ";")
		fen := parts[0]
		bestMoves := strings.Split(parts[1], " ")
		id := parts[2]
		retval = append(retval, BKTest{fen: fen, bestMoves: bestMoves, id: id})
	}
	return retval
}
